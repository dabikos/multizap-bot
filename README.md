# 🤖 MultiZap Bot для BSC

Telegram бот для автоматизации DeFi операций с поддержкой zap-in и exit операций на Binance Smart Chain (BSC).

## ✨ Возможности

- 🚀 **Zap-in операции** - покупка токенов + добавление в LP
- 🔄 **Exit операции** - продажа LP + получение BNB
- 🛡️ **MEV защита** - 7% slippage по умолчанию
- ⚡ **Оптимизированные газовые параметры** для BSC
...
1. Форк/загрузите проект в GitHub (без реальных секретов в `.env`).
2. Убедитесь, что в корне есть `render.yaml` (он создаёт 2 сервиса: Web и Worker).
3. В Render выберите New → Blueprint и укажите ваш репозиторий.
4. В настройках обоих сервисов задайте переменные окружения:
   - `TELEGRAM_BOT_TOKEN`
   - `PRIVATE_KEY`
   - `RPC_URL`, `CHAIN_ID`, `ROUTER_ADDRESS`, `FACTORY_ADDRESS`
5. Нажмите Apply — Render выполнит `npm ci && npm run compile` и запустит:
   - Web: `npm run web` (порт берётся из `PORT`)
   - Worker (бот): `npm run bot`

Примечания:
- `server.js` слушает `process.env.PORT || 3000`, что совместимо с Render.
- Не коммитьте реальные ключи — используйте `.env` локально и на Render в Secrets.
...
- [ ] Поддержка других сетей (Polygon, Arbitrum)
- [ ] Flashbots интеграция для Ethereum
- [ ] Web интерфейс
